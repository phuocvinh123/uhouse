...
export function renderSwiper() {
  ...
  const template = `
  {{#each items}}
    <div class='swiper-slide group z-1 relative'>
      <img data-src='{{this.imageSrc}}' alt='' class='lazy z-1 rounded-xl relative' />
      <div class='absolute top-2/3 bg-white opacity-80 max-w-full h-24'>
        <p class='font-sans font-bold text-red-700 mx-8 my-5 text-left xl:text-xl text-xs'>
          {{this.title}}
        </p>
      </div>
    </div>
...
export function renderHomeSectionLeft() {
  ...
  <div class='group overflow-hidden rounded-xl border hover:shadow-lg duration-300 shadow-md mb-4'>
    <div class='flex flex-col sm:flex-row group-hover:shadow-xl sm:h-full w-full'>
      <img
        data-src='{{this.imageSrc}}'
        alt=''
        class='lazy max-h-56 sm:max-h-full sm:max-w-52 md:max-w-72 z-10 border-none sm:rounded-l-xl gap-4 group-hover:scale-105 duration-500'
      />
      <div
        class='sm:rounded-r-xl rounded-b-xl sm:rounded-bl-none border-x-2 sm:border-x-0 border-b-2 sm:border-y-2 sm:border-r-2 border-orange-400 overflow-hidden py-6 gap-1 w-full h-full pl-4'
      >
        <div class='flex flex-wrap'>
          ...
          <div class='flex justify-center items-center pl-2'>
            <h3 class='text-base font-bold leading-5 text-red-600'>{{this.title}}</h3>
          </div>
        </div>
        <div
        ...
        >
          ...
          <div class='flex flex-wrap'>
            ...
            <h4 class='pl-3'>{{this.district}}</h4>
          </div>
          <div class='flex flex-wrap'>
            <div class='flex justify-center items-center'>
              <svg
          ...
          <div class='inline-flex'>
            ...
            <h4 class='pl-3 line-clamp-2'>{{this.address}}</h4>
          </div>
          <div class='inline-flex'>
            <div class='flex justify-center items-center'>
              <svg
            ...
            <h4 class='pl-3 line-clamp-2'>{{this.introduce}}</h4>
          </div>
        </div>
      </div>
    </div>
...
export function renderHomeSectionRight() {
  ...
  <div class='items-center text-center mb-8'>
    <h1 class='text-red-700 font-bold text-2xl'>{{this.title}}</h1>
    <h6 class='text-red-500 font-normal mx-2 xl:mx-10 text-sm mb-3'>
      {{this.Content}}
    </h6>
    <div>
      <img data-src='{{this.imageSrc}}' alt='' class='lazy h-80 w-full' />
    </div>
  </div>
{{/each}}`;
if(typeof Handlebars !== 'undefined'){
